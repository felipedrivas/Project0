...
public class UserDAO implements UserDAOInterface{
    ...
    @Override
    // Implementing deleteUser by ID
    public boolean deleteUser(int user_id) {
        // We will always try to connect to the DB first, before we can run any SQL
        try (Connection conn = ConnectionUtil.getConnection()) {
            // Create our SQL statement to delete the user based on the provided user_id
            String sql = "DELETE FROM users WHERE user_id = ?";

            // Use PreparedStatement to set the values
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, user_id);  // Set the user_id for the delete operation

            // Execute the delete command
            int rowsAffected = ps.executeUpdate();

            // If the rowsAffected is greater than 0, it means a row was successfully deleted
        ...
        // If something went wrong, return false (delete operation failed)
        return false;
    }
