...
public class UserController {
    ...
    // This Handler will handle DELETE requests to /users/{id}
    public Handler deleteUserHandler = ctx -> {
        // Extract the user_id from the path parameter
        int userId = Integer.parseInt(ctx.pathParam("id"));

        // Call the deleteUser method in the DAO
        boolean wasDeleted = eDAO.deleteUser(userId);

        // Check if the user was successfully deleted
        if (wasDeleted) {
            ctx.status(200); // OK - User deleted
            ctx.result("User deleted successfully.");
        } else {
            ctx.status(404); // Not Found - User not found
            ctx.result("User not found. Deletion failed.");
        }
    };
